...
pub fn main() -> Result<(), S3Error> {
    ...
    */
    ...
    for backend in vec![aws] {
        ...
        // bucket.
        let (_, code) = rt
            .block_on(bucket.put_object("test_file", MESSAGE.as_bytes()))
            .unwrap();
        ...
        let (data, code) = rt
            .block_on(bucket.get_object_range("test_file", 3, Some(6)))
            .unwrap();
        let string = str::from_utf8(&data)?;
        // println!("{}", string);
        assert_eq!(206, code);
        ...
        let (datas, code) = rt
            .block_on(bucket.get_object_multi_ranges("test_file", ranges))
            .unwrap();
        assert_eq!(206, code);
        ...
        rt.block_on(bucket.put_object_tagging("test_file", &[("test", "tag")]))
            .unwrap();
        println!("Tags set");
        ...
        let (_, code) = rt
            .block_on(bucket.put_object("random.bin", random_bytes.as_slice()))
            .unwrap();
        assert_eq!(200, code);
